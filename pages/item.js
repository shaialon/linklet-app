...
class Home extends Component {
  static async getInitialProps ({ query, req }) {
    console.dir(query, { colors: true, depth: null })
    ...
    try {
      // console.trace();
      // console.dir(itemId, {colors: true, depth: null});

      res = await db.getById({ itemId: itemId })
    } catch (e) {
      throw e
    }
    const isMobile = req
    ...
    return (
      <div className='home'>
        ...
        <main>
          <div className='list'>
            <h1>
              {item.name} - {item._embedded.cast.length}
            </h1>
            <img
              src={
                item.image.medium ||
                `//images.weserv.nl/?url=${item.image.medium
                  .replace('http://', '')
            ...
            <br clear='both' />
            <ul>
              {item._embedded.cast.map(member => {
                return (
                  <li>
                    <a target='_blank' href={member.character.url}>
